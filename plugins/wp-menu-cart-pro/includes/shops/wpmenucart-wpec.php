...
if ( ! class_exists( 'WPMenuCart_WPEC' ) ) {
	class WPMenuCart_WPEC {
		...
		public function cart_url() {
			return esc_url( get_option( 'shopping_cart_url' ) );
		}

		public function shop_url() {
			return esc_url( get_option( 'product_list_url' ) );
		}
		...
		*/
		...
		public function wpec_cart_ajax() {
			...
			$cart_contents = sprintf(_n('%d item', '%d items', $item_data['cart_contents_count'], 'wp-menu-cart-pro'), $item_data['cart_contents_count']);
