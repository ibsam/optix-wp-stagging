...
class Controller extends BooklyLib\Base\Component
{
    ...
    public static function calculateItemTotalsTaxes( $taxes, $item, $wc_cart_totals )
    {
        if ( property_exists( $item, 'object' )
            ...
            $userData = self::getUserBookingData( $item->object['bookly'] );
    ...
    public static function checkAvailableTimeForCart()
    {
        ...
        foreach ( WC()->cart->get_cart() as $wc_key => $wc_item ) {
            if ( array_key_exists( 'bookly', $wc_item ) ) {
                ...
                $userData = self::getUserBookingData( $wc_item['bookly'] );
                ...
                if ( self::checkIfServiceIsProvided( $userData ) ) {
                    ...
                    if ( $failed_cart_key !== null ) {
                        ...
                        foreach ( $userData->cart->getItems() as $cart_item ) {
                            ...
                            if ( $cart_item->getType() === BooklyLib\CartItem::TYPE_EVENT ) {
                                continue;
                            }
        ...
        if ( $lock + 30 < time() ) {
            ...
            foreach ( $wc_order->get_items() as $item_id => $order_item ) {
                ...
                if ( $data && ! isset ( $data['processed'] ) ) {
                    $userData = self::getUserBookingData( $data );
                    ...
                    // In case WC order has wp_user_id but Bookly customer has not
                    if ( ! $order->getCustomer()->getWpUserId() && $wc_order->get_user_id() ) {
                        $customer = new BooklyLib\Entities\Customer();
                        if ( $customer->loadBy( array( 'id' => $order->getCustomer()->getId() ) ) ) {
                            $customer->setWpUserId( $wc_order->get_user_id() )->save();
                        }
                    }
        ...
        if ( isset ( $wc_item['bookly'] ) ) {
            ...
            if ( isset ( $wc_item['bookly']['version'] ) && $wc_item['bookly']['version'] == self::VERSION ) {
                ...
                foreach ( $cart_items as $cart_item ) {
                    ...
                    $gift_card_type = null;
                    $event = null;
                    $event_ticket_type = null;
                    if ( $cart_item->getType() === BooklyLib\CartItem::TYPE_GIFT_CARD ) {
                        $gift_card_type = GiftCardType::find( $cart_item->getCartTypeId() );
                    } elseif ( $cart_item->getType() === BooklyLib\CartItem::TYPE_EVENT ) {
                    ...
                    if ( $cart_item->getType() === BooklyLib\CartItem::TYPE_GIFT_CARD ) {
                        ...
                        $codes['service_info'] = $event ? $event->getTranslatedInfo() : '';
                        $codes['service_name'] = $event ? $event->getTranslatedTitle() : __( 'Event was not found', 'bookly' );
                        $codes['service_price'] = $event_ticket_type ? BooklyLib\Utils\Price::format( $event_ticket_type->getPrice() ) : '';
                    } else {
        ...
        if ( count( $userData->cart->getItems() ) === 0 ) {
            ...
            if ( $failed_key === null ) {
                ...
                if ( $cart_item === null ) {
                    ...
                    $bookly['wp_user_id'] = get_current_user_id();
     ...
     */
    protected static function getUserBookingData( $cart_item )
    {
        $wp_user_id = array_key_exists( 'wp_user_id', $cart_item )
            ? $cart_item['wp_user_id']
            : null;

        return new BooklyLib\UserBookingData( null, $wp_user_id );
    }
